...
export default function StudentResult() {
  ...
  return (
    <>
      ...
      {/* <div className="text-lg h-screen overflow-visible bg-gray-100 max-w-5xl my-8 mx-auto">
                    <div className="text-lg w-full">
                      <img src={logo} className="text-lg d-block mx-auto h-48 w-48 my-2 object-center" alt=""="" />
                    </div>
                    <h3 id="current-term-header" className="text-lg font-extrabold text-center text-xl">
                      THIRD TERM STUDENT'S PERFORMANCE REPORT
                    </h3>
                    <div className="text-lg text-lg font-bold d-flex justify-content-between bd-highlight mb-3 max-w-5xl">
                      <div className="text-lg text-lg font-bold p-2 bd-highlight flex-grow">
                        <div>
                          NAME:{" "}
                          <span className="text-lg text-lg font-bold student-name-underline">
                            {result?.student.full_name}
                          </span>
                          GENDER:{" "}
                          <span className="text-lg text-lg font-bold student-gender-underline capitalize">{result?.student.gender}</span>
                        </div>
                        <div>
                          CLASS:{" "}
                          <span className="text-lg text-lg font-bold student-basic-data">{result?.student.current_class.name}</span>
                          SESSION:{" "}
                          <span className="text-lg text-lg font-bold student-basic-data">2020/2021</span>
                        </div>
                        <div className="text-lg text-lg font-bold performance-summary-table">
                          <table className="text-lg text-lg font-bold table table-bordered">
                            <thead className="text-lg text-lg font-bold thead-light">
                              <tr>
                                <th colSpan={5} className="text-lg text-lg font-bold perfomance-summary">
                                  PERFORMANCE SUMMARY
                                </th>
                              </tr>
                            </thead>
                            <tbody>
                              <tr>
                                ...
                                <td className="text-lg text-lg font-bold bolded-text">{result?.total_obtained}</td>
                                <td>PERCENTAGE</td>
                                <td className="text-lg text-lg font-bold bolded-text">{result?.percentage}%</td>
                                <td rowSpan={2} className="text-lg text-lg font-bold overall-remark">
                                  {result?.overall_remark}
                                </td>
                              </tr>
                              <tr>
                                <td>Total Obtainable:</td>
                                <td className="text-lg text-lg font-bold bolded-text">{result?.total_obtainable}</td>
                                <td>GRADE</td>
                                <td className="text-lg text-lg font-bold bolded-text">{result?.grade}</td>
                              </tr>
                            </tbody>
                          </table>
                        </div>
                      </div>
                      <div className="text-lg p-2 mt-5">
                        <img
                          src={result?.student.image}
                          className="text-lg img-thumbnail h-48 w-48 object-center object-cover"
                          alt=""=""
                        />
                      </div>
                    </div>
                <div className="text-lg text-lg font-bold container">
                  <table className="text-lg text-lg font-bold table table-bordered">
                    <thead className="text-lg text-lg font-bold thead-light">
                      <tr>
                ...
                <div></div>
                <div className="text-lg text-lg font-bold teacher-remark-section max-w-5xl">
                  Teacher's Remark
                  <div className="text-lg text-lg font-bold teacher-remark w-full">
                  {result?.teacher_remark}
                  </div>
                </div>
              Principal's Remark
              <div className="text-lg text-lg font-bold teacher-remark">
                {result?.principal_remark}
              </div>
              <div className="text-lg text-lg font-bold resumption-section max-w-5xl">
                Next Term Begins:{" "}
                <span className="text-lg font-bold border-b border-gray-700 text-base">{result?.next_term_begin_date}</span>
              </div>
      </div> */}
